...
/* apm - 13 buttons remote to move up and down speed (1-3) or reset it (2) */
/* apm - mix of case and if allows continuous running but ok increase-decresase in speed */
/* apm - this is the definitive version so far */
...
switch (My_Decoder.value) {
  ...
  case speedDown_13:
  {
    if (fStepsPerSecond > 100.0)
      {      
        fStepsPerSecond = fStepsPerSecond - 25.0;
        if (stepper1.speed() < 0) // to keep correct direction
            {stepper1.setSpeed(-fStepsPerSecond);}
        else {stepper1.setSpeed(fStepsPerSecond);}
      }
